...
function Home({apiData}) {
  ...
  const sendFormData = async () => {
    const response = await axios.patch(process.env.NEXT_PUBLIC_URL+`/api/admin/projects/${apiData._id}`, {
      title,
      tag,
      description,
      img:imgUploadName,
...
export async function getServerSideProps(context) {
    ...
    // console.log(pid);
    try {
      const res = await axios.get(process.NEXT_PUBLIC_URL + "/api/projects/" + slug);
      return {
        props: {
          apiData: res.data,
        },
